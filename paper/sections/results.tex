...
OSNet, by answering every registered pair without examining a crack. Its signal
is registration overlap: it assigns one constant score to registered image
pairs and $-\infty$ to unregistered pairs, so candidate images are ranked even
though crack pixels are never inspected. Much of the ranking and pairwise-$F_1$
margin is therefore \emph{which pairs align}; the
...
synthetic query is ranked against the wall's real instances only. Unlike
Table~I's \ours{} system, the synthetic-protocol \emph{Hybrid} folds a
structural skeleton comparison into the geometric baseline as a fallback plus
per-pair blend, scored by the same code as
